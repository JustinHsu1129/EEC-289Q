...
module model_top #(
    ...
    // NOTE ON INTER-CONV TRANSITIONS:
    // Because Systolic_Top completely isolates its internal input memory block (i_mem),
    // you must route Pool1 outputs directly to Conv2's internal memory matrix during simulation 
    // or custom synthesis runs using a hierarchical path assignment like below:
    //
    // always_ff @(posedge clk) begin
    //     if (pool1_wr_en) begin
    //         conv2_inst.i_mem.mem_array[pool1_write_cnt] <= pool1_wr_data;
    //     end
    // end
    ...
    assign conv2_ifmap_waddr = (pool1_wr_addr * CONV2_IC) + pool1_inst.ch;
