...
// Spatial ifmap tiling (packed channels share one ifmap word when IC < PeArrayRows).
localparam NumInTile = (InputFeatureMapChannelCount + PeArrayRows - 1) / PeArrayRows;
// Separate input-channel weight iterations (one weight line per IC per kernel tap).
localparam NumWeightIc = (InputFeatureMapChannelCount > PeArrayRows)
    ? NumInTile
    : ((InputFeatureMapChannelCount > 0) ? InputFeatureMapChannelCount : 1);
