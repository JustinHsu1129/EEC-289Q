...
logic [PeArrayRows-1:0] OutputFeatureMapWriteEnableDelay;
logic [PeArrayRows-1:0] AccumulatePartialSumDelay;
logic [PeArrayRows-1:0][OutputFeatureMapAddressBit-1:0] OutputFeatureMapAddressDelay;

logic OutputFeatureMapWriteEnable;

assign TestOutputOut = PartialSumData;
assign OutputFeatureMapReadAddressMux = TestCheckIn ?
    TestOutputAddressIn : OutputFeatureMapAddress;
...
always_ff @(posedge Clock) begin
    ...
    if (~ResetNegative) begin
        ...
        for (int i = 0; i < PeArrayRows; i++) begin
            OutputFeatureMapAddressDelay[i] <= '0;
        end
    end else begin
        ...
        for (int i = 1; i < PeArrayRows; i++) begin
            OutputFeatureMapWriteEnableDelay[i] <= OutputFeatureMapWriteEnableDelay[i-1];
            AccumulatePartialSumDelay[i] <= AccumulatePartialSumDelay[i-1];
            OutputFeatureMapAddressDelay[i] <= OutputFeatureMapAddressDelay[i-1];
        end
    end
end
