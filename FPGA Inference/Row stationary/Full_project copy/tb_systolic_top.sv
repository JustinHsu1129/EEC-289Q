...
parameter int PeArrayRows = 1; // Physical PE rows holding stationary IFMAP rows
parameter int PeArrayColumns = 6; // Physical PE columns
...
parameter int InputFeatureMapChannelCount = 1; // Logical input channels
...
localparam int InputFeatureMapDepth = TotalPixels * ReductionSteps;
localparam int WeightDepth = ReductionSteps * NumOutTile;
...
localparam int OutputFeatureMapAddressBit = (OutputFeatureMapDepth <= 1) ? 1 : $clog2(OutputFeatureMapDepth);
...
initial begin
    ...
    $display("\n>> Processing %0dx%0d Image with %0d input channels, %0d output channels, and a %0dx%0d row-stationary PE array.",
...
// Mock memory modules (address map matches Mem_ctrl)
...
module ifmap_mem #(
    ...
    parameter OutputFeatureMapWidth = 26,
    parameter OutputFeatureMapHeight = 26,
    ...
    input logic [$clog2(Depth)-1:0] ExternalWriteAddress,
...
integer ReductionAddress;
integer OutputPixelAddress;
integer RowTile;
integer KernelColumn;
integer InputChannel;
integer OutputPixelRow;
integer OutputPixelColumn;
integer KernelRow;
integer RawAddress;
...
module ofmap_mem #(
    parameter Width = 32, // Output remains the same (OutputFeatureMapBitWidth * PeArrayColumns)
    parameter Depth = 2048,
