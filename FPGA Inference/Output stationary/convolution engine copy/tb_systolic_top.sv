...
localparam int WeightDepth = WeightWidth * WeightHeight * NumInTile * NumOutTile;
...
initial begin
    ...
    $display("\n>> Processing %0dx%0d Image with %0d input channels, %0d output channels, and a %0dx%0d output-stationary PE array.",
...
always_ff @(posedge Clock) begin
    ...
    for (int Row = 0; Row < PeArrayRows; Row++) begin
        ...
        // Lane r must match PE_Array row r (InputFeatureMapDataIn[r]).
        QLane = Row;
...
always_ff @(posedge Clock) begin
    ...
    InTile = PackedInputAddress % NumInTile;
    KernelAddress = PackedInputAddress / NumInTile;
    if (KernelAddress < WeightWidth * WeightHeight) begin
        for (int Row = 0; Row < PeArrayRows; Row++) begin
            LogicalInputChannel = InTile * PeArrayRows + Row;
            RawAddress = KernelAddress * InputFeatureMapChannelCount + LogicalInputChannel;
            if (LogicalInputChannel < InputFeatureMapChannelCount) begin
