...
module model_top #(
    ...
    logic        pool1_wr_en;
    logic [2:0]  pool1_current_ch; // FIX: Local net to hold pool1 channel ($clog2(6) = 3 bits)
    ...
    logic        pool2_wr_en;
    logic [3:0]  pool2_current_ch; // FIX: Local net to hold pool2 channel ($clog2(16) = 4 bits)
    ...
    // FIX: Swapped out "pool1_inst.ch" for synthesizable local net "pool1_current_ch"
    assign pool1_rd_data   = conv_relu_to_pool18(
        conv_with_bias(conv1_test_out[pool1_current_ch * 32 +: 32], conv1_bias[pool1_current_ch])
    );
    ...
    pooling_top #(
        ...
        .wr_en(pool1_wr_en),
        .current_ch_out(pool1_current_ch) // FIX: Bound output port to top net
    );
    ...
    // FIX: Swapped out "pool2_inst.ch" for synthesizable local net "pool2_current_ch"
    assign pool2_rd_data   = conv_relu_to_pool18(
        conv_with_bias(conv2_test_out[pool2_current_ch * 32 +: 32], conv2_bias[pool2_current_ch])
    );
    // FIX: Swapped out "pool1_inst.ch" for synthesizable local net "pool1_current_ch"
    assign conv2_ifmap_waddr = (pool1_wr_addr * CONV2_IC) + pool1_current_ch;
    ...
    pooling_top #(
        ...
        .wr_en(pool2_wr_en),
        .current_ch_out(pool2_current_ch) // FIX: Bound output port to top net
    );
