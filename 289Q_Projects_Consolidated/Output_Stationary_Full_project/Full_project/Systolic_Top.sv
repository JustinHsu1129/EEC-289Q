...
// -------------------------------------------------------------------------
// Pipeline Registers for Control & Timing Alignment (Fixes the "Stuck at 7" error)
// -------------------------------------------------------------------------
...
// Synchronous block to delay control signals by 1 cycle to match PE array data latency
...
ofmap_mem #(
    ...
    .WriteAddress(OutputFeatureMapAddress_reg),     // Delayed address
    .WriteEnable(OutputFeatureMapWriteEnable_reg),   // Delayed write enable
    // .Accumulate(1'b0),                            // FIXED: Removed non-existent port connection
