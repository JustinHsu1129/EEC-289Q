...
module model_top #(
    ...
    // Channel trackers (for FSM boundary monitoring)
    logic [10:0] pool1_write_cnt;
    logic [3:0]  pool1_ch;
    logic [8:0]  pool2_write_cnt;
    logic [4:0]  pool2_ch;

    // Real-time hardware channel index wires connected directly to the sub-modules
    logic [2:0]  pool1_actual_ch;
    logic [3:0]  pool2_actual_ch;
    ...
    always_comb begin
        ...
        case (state)
            ...
            ST_POOL1:     if (pool1_write_cnt == 1176) next_state = ST_CONV2; 
            ST_CONV2:     if (conv2_finish) next_state = ST_POOL2;
            ST_POOL2:     if (pool2_write_cnt == 400) next_state = ST_FC_STREAM;  
    ...
    // FIX: Pure combinational matching utilizing the real-time wire mapped out of the sub-module
    assign conv1_test_addr = pool1_rd_addr;
    
    always_comb begin
        pool1_rd_data = conv_relu_to_pool18(
            conv_with_bias(conv1_test_out[pool1_actual_ch * 32 +: 32], conv1_bias[pool1_actual_ch])
        );
    end
    ...
    pooling_top #(
        ...
        .wr_en(pool1_wr_en),
        .current_ch_out(pool1_actual_ch) // EXPLICIT PORT CONNECTION
    );
    ...
    // ============================================================
    // FIX: Pure combinational matching utilizing the real-time wires mapped out of both sub-modules
    assign conv2_test_addr   = pool2_rd_addr;
    assign conv2_ifmap_wdata = pool1_wr_data;

    always_comb begin
        pool2_rd_data = conv_relu_to_pool18(
            conv_with_bias(conv2_test_out[pool2_actual_ch * 32 +: 32], conv2_bias[pool2_actual_ch])
        );
        conv2_ifmap_waddr = (pool1_wr_addr * CONV2_IC) + pool1_actual_ch;
    end
    ...
    pooling_top #(
        ...
        .wr_en(pool2_wr_en),
        .current_ch_out(pool2_actual_ch) // EXPLICIT PORT CONNECTION
    );
