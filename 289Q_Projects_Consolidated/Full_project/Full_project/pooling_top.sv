...
module pooling_top #(
    ...
    input  logic clk,
    input  logic rst_n,
    output logic [9:0]  rd_addr,       // FIX: Changed from input to output
    input  logic [WIDTH-1:0] rd_data,
    output logic [7:0]  wr_addr,
    output logic [WIDTH-1:0] wr_data,
    output logic        wr_en,
    
    // Output port to expose the internal channel variable
    output logic [$clog2(CHANNELS)-1:0] current_ch_out 
);
