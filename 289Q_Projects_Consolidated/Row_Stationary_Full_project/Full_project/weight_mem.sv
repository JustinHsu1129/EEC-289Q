...
module weight_mem #(
    ...
    // FIXED: Extracted address division mapping out of the synchronous process
    int OutTile;
    ...
    always_comb begin
        OutTile          = int'(Address) % NumOutTile;
        ReductionAddress = int'(Address) / NumOutTile;
    ...
    always_ff @(posedge Clock) begin
        ...
        for (int Row = 0; Row < PeArrayRows; Row++) begin
            ...
            automatic int RawAddress = (KernelRow * WeightWidth + KernelColumn) *
            ...
            if (InputChannel < InputFeatureMapChannelCount && KernelRow < WeightHeight) begin
                for (int Lane = 0; Lane < PeArrayColumns; Lane++) begin
                    automatic int LogicalOutputChannel = OutTile * PeArrayColumns + Lane;
