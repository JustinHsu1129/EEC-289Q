...
logic [PeArrayRows-1:0] WeightMacValidShift;

always_ff @(posedge Clock) begin
    if (~ResetNegative) begin
        OutputFeatureMapValidOut <= {PeArrayColumns{1'b0}};
        WeightMacValidShift <= '0;
    end else begin
        WeightMacValidShift[0] <= WeightMacEnableIn;
        for (int i = 1; i < PeArrayRows; i++) begin
            WeightMacValidShift[i] <= WeightMacValidShift[i-1];
        end
        OutputFeatureMapValidOut <= {PeArrayColumns{WeightMacValidShift[PeArrayRows-1]}};
    end
end
